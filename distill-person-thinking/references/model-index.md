...
本轮已纳入的人物不再重复列为待核验候选;同一人物的未收录观点仍不能从记忆或上游名称直接补成模型.其他新发现人物只有在用户授权扩展且达到证据合同后才加入.

Nuwa 的 `x-mastery-mentor` 是主题导师,不属于人物模型,不纳入集合.社区仓库的已吸收能力,已覆盖项与未采用理由统一见[来源与复用边界](source-policy.md).Star 只用于发现线索,不作为证据或适用性评分.
